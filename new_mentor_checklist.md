...
| A3 | bond シナリオ 全20話 | `masters/mentor-<id>-bond-*.brief.json` → emit | 構成の型: **師弟編1〜12話**（12話=フィナーレ・読了で覇道編移行）／**覇道編13〜19話**／**20話=エピローグ（tournament_won 9 解禁・優勝後の物語）** | ❌(2話のみ) | ❌ |
...
| A6 | 対局セリフ | `characterVoiceMaster.js`（voice-lines 経由） | matchStart/agari/damage/matchEnd ＋ 局中反応。詩玥のみ本実装（`［テンプレ］` 6箇所） | ❌ | ❌ |
...
| B2 | 覇道編移行点 | `MENTOR_FINALE_SCENARIO`（=師弟編最終章ID） | ❌ | ❌ |
| B3 | エピローグ章（読了でスタッフロール） | `MENTOR_EPILOGUE_SCENARIO` | ❌ | ❌ |
| B4 | 技Lvの軌跡（5→10・シナリオ同期） | `MENTOR_SKILL_TRACK` | ❌ | ❌ |
...
| B7 | スキルLvテーブル本設計 | `skillLevelMaster.js`（lv-iron-guard 等4本が仮=runtimeParams空）＋能力側の params 対応（Phase 7結線） | ❌（skill-smith で設計→[[skill-transcendence-policy]] 超越帯=相棒の能力が宿る） | ❌ |
| B8 | 師匠ボイス（ホーム） | `mentorVoiceMaster.js` | greeting/restTalk/praise=✅。**battle quips / 昇段 / 大一番口上 / duo誘い / 雀荘 =テンプレ**。phase:hadou行・cleared行・treasuresMin行・bondMin高絆行（口調崩れ）も拡充 | 同左 |
...
| B12 | スタッフロールの締めの一言 | `creditsMaster.lastLineByMentor` | ❌（未定義は default） | ❌ |
