...
- ✅ **新効果バッチ（実装済み）**：採用リストを実装。
  - **PT3人固定**：枠増加バフ(add-bench)廃止。**深度値上がり**：HP/攻撃/防御バフの付与値を階層で増幅（depthGrowth0.025/階）。
  - **条件付きバフ4種**：波に乗る(連勝×与ダメ)／火事場(瀕死で与ダメ↑被ダメ↓)／倍返し(被弾後の和了↑)／背水のリーチ(リーチ中の和了↑)。`rlApplyDynamicBuffs` で対局中に評価。
  - **アイテム3種**：復活の霊薬(ダウン者を全員HP100%復活・no-revive例外)／不死鳥の羽(ラン1回トビ救済HP1)／先見の札(次の層を表示)。
  - **交代マス「流派の門」**(kind:recruit)：光貨50で候補3人から1人を迎え1人を送り出す（PTは3人のまま）。`recruitCandidates`/`swapPartyMember`。
...
- ✅ **HP成長カーブ（青天井＋割合/絶対値棲み分け＋館の気脈）／難易度カーブ＋チューニング行動ログ（実装済み）**：
  - **HP上限は撤廃（青天井）＋HPバフを「割合／絶対値」で棲み分け**：当初 `hpMulCap=1.9` で頭打ちにしたが**+90%で凍結→「取っても増えない＝壊れて見える」**体感バグに。上限は撤廃（`hpMulCap=null`・検証ノブとして残置）。**ただし真の暴走源は別**だった＝**割合(×)HPが maxStacks を回避して無制限に積めた**こと：ショップ「厚みの護符」(合成カード`_shop-maxhp`)・祠「光貨を捧げる」・イベント「点棒の山」が `maxHpUp` を直接適用し、ショップ/祠は何度でも繰り返せる→深度値上がりと複利で **hpMul が数十億倍・実HP兆単位**に膨張（育成完了greedyが GUARD200 に張り付く＝事実上無限スポンジ）。
    - **対策＝割合と絶対値の棲み分け**（バフ値は鈍化させない・各カードは満額）：**割合(`maxHpUp`)は epic 以上の少数のみ**（`maxhp-up-epic`×2 / `fortress`×1＝複利は有界）。**common・ショップ・祠・イベントは絶対値(`maxHpAdd`)へ**＝固定HP点（深度値上がりで額は増えるが**線形＝複利爆発しない**）。`maxHpAdd` は `cardEffects.js` に新設（`run.mods.hpAdd` に累積・現在HPも同量底上げ）。
    - 結果（実測）：hpMul は **3.7〜4.3倍で有界**化、無限テール(GUARD200張り付き)が消滅。バフは満額のまま。
  - **難易度カーブ「うまく戦えばいける／どんどん難しくなる」**：被ダメ青天井を `floorDmgStart 6→8`・`floorDmgSlope 2.4→4.0` に再校正。**F8まで被ダメ深度なし＝地力で抜けられる**／**F10×9・F20×49・F30×89 と深層で急峻**。生存曲線は序盤フラット→深層で脱落加速（F12:2%→F17:11%）。無策(none)は14前後で浅い＝下手だと進めない。
  - **館の気脈（味方の最大HPが少しずつ上がる）**：`growMaxHp`＝1フロア進むごとに最大HPを基礎HP比 `FLOOR_HP_GROWTH=0.03` 底上げ（線形・現在HPも同量）。敵HPが階層で増える(`floorEnemyHp`)のに追従させ、**攻撃特化ビルドでも深層で「相手の合計HPを上回る」＝制圧ボーナスを狙える**（F30で味方3人≈5700 > 敵2人4648）。0.05以上は無限化するため0.03（控えめ）に。`run.party[].baseHp`（初期HP最大）を底上げ基準に保存（serialize対応）。
    - 総合結果（実測）：育成完了greedy **mean 63.7→26.5・max 201(∞)→140（必ず終わる）**、中央値20・**バフ効き×1.43 維持**（HP底上げで好プレイが報われる）。`--assert` 7/7。残る深掘り(max140)は「最強キャラが毎フロアの1局を取り続け＝被弾しない」由来で、HPではなく勝率の問題（許容＝うまく戦えば深く行ける“ご褒美”）。
