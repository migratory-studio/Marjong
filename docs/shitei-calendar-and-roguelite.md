...
- ✅ **バイオーム（"層"）システム＋層入場演出（実装済み）**：10階＝1層。塔の景色（背景）と固有モディファイアが帯ごとに変わる（楼光＝記憶を映す塔）。
  ...
  - モディファイアは**既存ノブに掛けるだけ**：`dmgTakenMul`(被ダメ深度)・`dmgDealMul`(与ダメ深度)・`coinMul`(光貨)・`regenMul`(踏破回復)・`enemyLvAdd`(敵Lv)・`draftBias`(レア度)。
  - 初期6層：平穏の廃墟(中立)／崩落の坑(basement・被ダメ+50%/光貨×1.4)／狂騒の祭(festival・光貨×3/被ダメ+25%)／温泉郷(ryokan・被ダメ−35%/回復+60%)／黄昏の屋上(rooftop・与ダメ+25%)／喧噪の都(city・敵Lv+1/レア度↑)。背景は全て実リソース。
  ...
  - 設計：ベース深度倍率はなだらかに保ち、ハード層が山場を作る（全体なだらか＋層で緩急）。`--assert` 7/7・回帰PASS。
