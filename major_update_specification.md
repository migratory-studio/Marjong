...
##### B2. 師匠タイマン（二人打ち＝本気）
- 二人打ちタイル → **「オート / 本気」** を選択。
  - オート＝既存 autobattle vs 師匠（師匠 role で param 配分）。
  - 本気＝**二人麻雀（futari・実装済）で vs 師匠**（`voiceSet:"shugyo"`）。新規モード構築は不要、A の起動に乗せるだけ。
- **師匠ごとに打ち筋**（role 依存）。**惜敗で伸び**＝着順/点差が僅差なほど param 経験 UP（敗北を学びに）。
...
> ※ Phase 4B（初級大会＝C／クリア評価＝D）は別途仕様確定する。本節は §20 Phase 4A/4B の「鍛錬／対局起動」部分を更新（上書き）する。
