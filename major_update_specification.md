...
**訓練の「調子」（実装版）**: 各訓練は **大成功 / 成功 / 無難 / 失敗** の結果を引き、伸び量に倍率
（大成功 ×2.2 / 成功 ×1.5 / 無難 ×1.0 / 失敗 ×0.34、主は最低 +1＝失敗なし路線）が掛かる。
**メンタルが高いほど無難・失敗が減り、成功・大成功へ寄る**（対局オートと同じ「ブレを抑える」性格）。
結果に応じて**師匠が一言返す**（双方向の愛着フック）。確率・倍率は `TRAIN_OUTCOMES` / `rollTrainOutcome`（progressionService）に集約。
