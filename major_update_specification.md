...
- 二人打ちタイルは**師匠との二人麻雀タイマン**になる（東南戦・futari・実装済エンジン／`voiceSet:"shugyo"`）。
  **持ち点＝点棒＝HP**：弟子は**今の HP（`avatarHpCurrent`）を賭けて**打つ。**対局後の残点が新しい HP**（`avatarHpCurrent`、上限＝最大HP）。HP が僅少（<1000）なら打てず「休んでから」。
- **師匠は格上**：師匠の持ち点＝**フリー対戦の HP（キャラ既定 `stats.startingPoints`）が初期値**で、**師弟シナリオの進捗（既読話数）で強化**（`mentorDuoHp`：base＋既読数×係数・上限あり）。最初は大きな格差（例 詩玥14000 vs 弟子数千）。
  ※**覇道編以降の「一緒に育成＝共闘」はペア/団体大会（詩玥＋弟子）で表現**（タイマンは育成期の“壁役”）。
- 開始時に **「オートで打つ / 本気で打つ」** を選ぶ＝**同じ一局を AI 自動打ち / 自分で手動**のどちらで打つか（autoPlay 流用・対局中トグル可）。
- **勝ち＝HP を増やせた（師匠から点を奪えた）こと**（格上ゆえ総合超えはほぼ不可）。`applyDuoResult` は `closeness`（`finalPoints/賭けたHP`）でメンタル(主)・読み(副)を段階化（完敗でも最低 +1）。HP 増で調子↑＋ソウル。リザルトは **HP の増減（before→after）** を見せる。1 行動消費。
- TODO: 師匠ごとの打ち筋差（現状は師匠の素の能力／AI のまま）、師匠側の対局中ボイス（現状 `shugyo` は弟子=詩玥が打ち手のときのセット）。

##### 決定値（チューニング・実装時に確定）
- 持ち点: 単発の本気（4人）＝**固定 25,000**（勝敗・点差を見る）／**師匠タイマン＝弟子の現 HP を賭け、結果を HP に反映**（点棒＝HP）／**宝大会＝弟子は育成HP(avatarHpMax)・相手は oppLv連動HP で毎節開始（節リセット・トビ脱落なし・本HPは減らさない）**（§4.6.10）。
...
**形式** `format`: `solo4`(個人四麻) / `solo3`(個人三麻) / `pair`(ペア2人) / `team`(団体3人) / `final`(九蓮宝燈＝会場でキャラ別に人数確定)。
...
順序は **九蓮宝燈＝全キャラ最終で固定**、ティアは基本 T1→T2→T3（同ティア内はキャラ自由）。優勝で `records.treasures` に宝 id を記録。
**キャラ別キャンペーン（`mentorCampaignMaster`）**: 師匠ごとに **9 宝の挑戦順＋各到達時の oppLv（敵強度カーブ）** を定義。
`nextTreasureStep(mentorId, records.treasures)` で「次に挑む宝」を出し、**CUP は次の宝へ自動進行**（優勝で `records.treasures` に記録→次の宝へ）。
九蓮宝燈の会場形式はキャラ別 `finalFormat`（詩玥＝pair／ビビ＝team／ルイナ＝solo4）。CUP ボタンに次の宝名・形式・ティアを表示。
**実装済みラン**: `solo4` / `solo3` / **`pair`** / **`team`**（および九蓮宝燈の `finalFormat`＝pair/team/solo4）すべて対応。`runnable` は全形式 true。

**宝の大会＝M リーグ制・ユニット制（全手動・実装版）**（ホーム CUP「宝への道」から）。リーグは常に **8 ユニット** で競う。
- **出場者数を形式で固定**（`ENTRANTS_BY_FORMAT`）：**個人 8 名（1 人×8）/ ペア 16 名（2 人×8）/ 団体 24 名（3 人×8）**。九蓮宝燈は会場形式に追従。
  - `UNIT_SIZE_BY_FORMAT`＝個人1/ペア2/団体3、`unitCount` は常に 8、`UNITS_AT_TABLE_BY_FORMAT`＝個人4/ペア2/団体4（卓に着くユニット数）。
  - 大会個別の上書き `tableUnits`（マスタ）：**大三剣杯＝3チーム着卓**（「三」の宝＝卓が3人＝**三人打ち（三麻）**になる。席数はユニット数で決まる）。
  - **個人戦は四麻と三麻を半々**：門前開鍵杯＝四麻（solo4）／**清一器杯・無双冠杯＝三麻（solo3・卓3人・ウマ[+30/±0/△30]）**。九蓮宝燈が個人形式に解決されるとき（ルイナ）は四麻（`finalFormat: solo4`）。
- **自ユニット編成**（`buildDeshiUnit`）：個人＝弟子のみ／ペア＝**弟子＋師匠**／団体＝**弟子＋師匠＋仲間**（`ALLY_BY_MENTOR`：ビビ=焔(homura)・詩玥=マモリ・ルイナ=ドラニエ／正典準拠）。
- **ライバルのユニット化**（`tournamentRivalMaster.rivalUnits`）：代表（lead）＝ネームド or モブ＋仲間をモブで充足。ネームド比率はティアで増加（T1=1/T2=3/T3=全員、`NAMED_BY_TIER`）。
- **持ち点＝HP（点棒＝HP）**：**弟子ユニットは育成した HP（`avatarHpMax`）** で打つ＝育成成果が大会の持ち点に直結。**相手ユニットは大会の oppLv 連動 HP**（`oppHpForLv`）＝難易度が相手の体力に出る（格上ほど分厚い）。
  **節ごとに満タンで開始・トビ脱落なし**（`avatarHpCurrent`＝本HP は減らさない＝失敗しないサクセス育成）。runHp 持ち越しは不採用。
- **節＝T1/T2は東風・T3は半荘**（`TOURNAMENT_TIER.rounds`。半荘×複数節は体感が長すぎたため T2 も東風化）。毎節は卓に `unitsAtTable` ユニットが着卓し、**弟子は必ず参加・他ユニットはローテーションで入れ替わる**（`seatUnitsFor`）。
  個人＝`launchHonestMatch`、ペア＝`launchPairTournamentMatch`（2 対 2）、団体＝`launchTeamTournamentMatch`（4 チーム。大三剣杯のみ 3 チーム＝三麻卓）。結果は `honestCtx.onResult` でユニット最終持ち点を返す（pair/team の対局終了に大会分岐を追加）。
- **採点**：各節 **素点＝(ユニット最終−自分の開始HP)/1000**（＝その節の稼ぎ）。**ウマは「増減（稼ぎ）の順位」で配る**（`UMA_BY_UNITS`：4組=[+50/+10/△10/△30]、3組=[+30/±0/△30]、2組=[+15/△15]）。
  ＝育成は「持ち点の厚み＝攻めの余裕・トビにくさ」として効き、**宝は打ち回しの累積で決まる**（育成だけで勝ち確定にしない）。**卓に居ないユニットは別卓扱いで擬似ポイント加算**（`simAbsentLeaguePt`）＝全員の累積が毎節動く。
- **出場ゲート**：相手評価（§4.6.2）が **大劣勢なら門前払い**（`tournamentGate`）。
- **専用演出**：開幕前に**要項画面**（`showTournamentBriefing`：ルール／優勝条件／ライバル紹介。ネームド最大4枚＋集約で無スクロール）。対局 intro に**大会リボン**＋ライバル口上、対局中は**左上 HUD**（大会名／節／前節までの累積順位）。
- **順位表演出（ヒリヒリ）**：各節後に**累積ポイントの順位表（8 ユニット）**（この節の増減・弟子ハイライト・カウントアップ）を提示し、**次の節／退く**を選ぶ。
- **クリア（D）**：全節終了後の**最終累積順位（ユニット）**で評価（`applyLeagueResult`）。順位→**評価ランク**（満貫級が下限・8位も満貫級にクランプ・§4.5.2）＋**継承（`wallet.meta`）＋ソウル**。
  **最終1位＝優勝**で **`records.tournamentsWon++`** ＋ `records.treasures` に宝 id 記録（＝シナリオ `tournament_won` 解放）。**失敗なし**（持ち点＝HP は節ごと満タンで開始・トビても脱落せず・本HPは減らさない）。
- ※**雀荘巡りの「大会中の雀荘」はオート（autobattle）のまま**でよい（手触りの軽い枠）。

**未実装（今後）**: 大会の**複数日消費**（§4.5.3）、**周回クリア評価**（§4.5.2 の日数×勝率→引退/見送り→継承）の本実装、
`mobLvBand` 連動・しきい値チューニング・大会の得点推移グラフの節間自動表示。
...
- **Lv1〜10 はソウルのみで強化可**（師弟編の主育成帯。Lv10＝最大HP 26,000）。
- **Lv11〜20 は「宝の解禁制」**：宝（大会優勝）を 1 つ獲るたびに上限が 1 つ開く（`avatarLevelMaster.requireTreasures`。Lv11=宝2 〜 Lv18+=宝9）。物語＝「宝を集めるほど打ち手の器が広がる」。HP は Lv20＝65,000 までグンと伸びる（大会の宝は増減で決まるため勝ち確にはならない）。
- 経済両立: 段位（宝数）で雀荘の勝ちご祝儀が +8%/宝 されるため（§4.6 雀荘）、キャラLv11〜20（計21,000）とスキル超越帯（計28,800）が終盤までに両方完走できる。回帰＝`test/leveldesign.mjs`。
...
#### 超越帯の能力設計（予約）— 守り（danger-sense）：聴牌嗅覚・覚醒

`lv-danger-sense` の超越帯（Lv6〜10）の設計。**守りビルドの「決定打」**として、危険感知を「推定」から「確定」へ昇華させる。育成（ビビの `tmpl-danger-sense` 経路）で到達。マモリの誓い「放銃しない」の絶対形。

- **能力「聴牌嗅覚・覚醒」**：ゲーム中1回。誰かがリーチしている間に発動すると、その**リーチ者の待ち（あたり牌）を完全に開示**する（発動した局の終わりまで持続＝リーチ後は手が固定で待ち不変）。
- **テーマ／超越帯パターン**：基準帯＝「気配であたり率を推定」（`estimateDangerInfo`）→ 超越帯＝「待ちを確定で視る」。超越帯の**3つ目のパターン＝「能力自身が極まる」**（詩玥＝相棒の能力が宿る／凌雲＝相棒の哲学が宿る／マモリ＝自分の極致）。
- **実装方針**：`PROVIDE_DANGER_INFO` のチャネルを流用し、推定値の代わりに対象リーチ者の**実際の待ち（`waits()` で算出）を最高位＝確定**で返すだけ。伏せ手 `opp.hand` を読む（＝異能で覗く）。リーチ検知は `opponents().some(o=>o.riichi)` で十分（専用イベント不要）。発動回数＝game-scoped `maxCharges:1`。
- **強さの担保**：純守備・対象1・1回・**育成専用（フリー対戦＝対戦相手には出ない）**＝点を生まず暴れない。1回は安全側の縛り（Lv10で2回へ緩めても壊れない）。
- **差別化**：詩玥の継承（lucky-draw 超越帯の `dangerTier`）には**乗せない**。詩玥＝推定のまま／覚醒は「守り本道」の専売。

**要決定ノブ（未確定・実装着手前に詰める）**：

1. **Lv6〜10の配分**：Lv6で覚醒解放（1回・リーチ待ち確定）→ 間で推定精度／対象拡張 → Lv10で2回、を骨子に。例＝Lv8「鳴き（副露テンパイ）の待ちも覚醒対象」、Lv10「2回＋超危険は常時確定表示」。刻みは要確定。
2. **複数リーチ時**：発動で「その時点の全リーチの待ちを一括開示」（推奨）か、1人ターゲットか。
3. **基準帯 Lv1〜5**：現 `danger-sense`（推定3段階）のまま Lv5＝完成（＝現マモリ）でよいか。覚醒は純粋に Lv6〜10 の追加。
4. **UI**：覚醒の待ちは「あたり」確定の専用マーク（推定の赤とは別色）で“視え方が変わる”体感を出す。
5. **マモリ本人への波及**：本機能は育成の守りビルドのペイオフで、フリー対戦のマモリ自体は不変。団体戦の味方マモリ等にも見せたい場合は別途 `asMentorMatchChar` 経由の結線が要る（任意・要判断）。
...
Phase 5 で追加する。**※ 2026-06 時点で Phase 5（アイテム）は当面スキップ＝後回し（§20 Phase 5 の注記参照）。**
...
> **【当面スキップ】（2026-06 判断）**: アイテムは後回し。理由＝放置すると「ただの数値バフ」になりがちで、本作の主軸（キャラ愛着・体感）への寄与が薄いため。
> 再開する場合は §15 とこの節を参照。設計時は「詩玥との関係性／体感に効くか」を必ず一度通すこと。
