...
- **HP（点棒）は試合中、減る一方のダメージバー**。局を取れば被害ゼロ、負ければ（放銃／被ツモ／着順）でダメージ。
  ＝自分のバーは増えない（点のやりとり演出は場＝他家バーで見せる）。
...
> 演出：既存フリー対戦の **HP やりとりアニメをデフォルメ／簡略化して連続再生**する（4 人の HP バー＋
> ロン／ツモで点棒がスッと移動）。新規のフル描画は不要で、オート用の軽い再生器を作る。

コマンドの効き（係数は §25 同様チューニング値。HP は減る一方なので「＋」＝被害回避／好着順の意）:

| コマンド | 局を取れたら | 取れなければ | 使いどころ |
| --- | --- | --- | --- |
| 押す（火力） | 被害ゼロ＆好着順へ | 大ダメージ（放銃） | 優勢・火力高 |
| 引く（守備） | 利得は小 | 守備で大幅軽減した小ダメージ | 劣勢・守りたい |
| 様子を見る（読み） | 被害ほぼ無し＋次手の精度↑ | 小ダメージ | 立て直し |
| 次ラスで！（勝負勘） | 一気に好着順 | 致命傷 | ラス局・一発逆転 |
