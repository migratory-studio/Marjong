...
| 座学 | 1 | −小 | — | +微 | 守備・読み系スキル経験（安全・地味） |
| 鍛錬(CPU) | 1 | −中 | ++ | + | 攻撃系スキル経験（ソウル稼ぎ） |
| 師匠と 2 人打ち | 1 | −中(手心) | + | +++ | スキル経験 ＋ 素が滲む（愛着の主戦場） |
...
- 各 育成コマンド（座学・鍛錬・二人打ち・雀荘巡り）が、どのパラメータを伸ばすかの対応は要詰め。
...
- **局単位のターン制**。毎局「押す／引く／様子を見る／次ラスで！」を選ぶ短い RPG 戦。
- 自パラメータ × 相手 × 乱数で**点棒（＝HP）が増減**する＝「互いがぶつかり合う」。HP＝点棒・`runHp`（§14.3）と地続き。
- 速度で先制、メンタルで安定（劣勢でのブレを抑える）。
- HP 0 で敗退。**大会＝この連戦**。オートバトルの勝敗は**クリア評価（日数×勝率, §4.5.2）**の勝率に乗る。
...
- **本気で勝つとボーナス**（ソウル多め＋パラメータ経験など）。オート任せより実入りが良くして「ちゃんと打つ」動機にする。報酬詳細は要詰め。

#### 4.6.5 大会出場ゲート

- 各大会に必要ライン（評価が**大劣勢なら門前払い＝出ても 100% 負け**）。
- 育成でラインを満たしてから挑む＝サクセスの王道動線。`mobLvBand`（§4.5.4）と評価式で算出する。

#### 4.6.6 要詰め

1. 育成コマンド → どのパラメータを伸ばすかの対応表
2. 総合評価の算出式（パラメータ総合の重み・6 段階のしきい値）
3. オートバトルの点棒増減式（コマンド×相手×乱数・速度/メンタルの効き）
4. 本気勝利ボーナスの中身と倍率
