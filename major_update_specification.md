...
**C 初級大会**（ホーム CUP「宝への道」から）。`tournamentMaster`（初級大会＝3 戦／相手 Lv 2／敵 HP 8,000）。
- **出場ゲート**：相手評価（§4.6.2）が **大劣勢なら門前払い**（`tournamentGate`）。
- **中身（宝の大会＝全手動）**：**実麻雀を N 戦連戦**（`launchHonestMatch` を毎戦・4人 東風）。弟子は **runHp を持ち越して着席**
  （`startPoints=runHp`、相手は 25000 のモブ）、各戦の残点が次戦の持ち越しになる。**HP 0（トビ）で敗退**、各戦後に**次戦／撤退**を選べる
  （撤退は完走扱いにならない）。※**雀荘巡りの「大会中の雀荘」はオート（autobattle）のまま**でよい（手触りの軽い枠）。
- **クリア（D）**：完走で **勝率（連対数/戦数）→ 評価ランク**（満貫級→跳満級→倍満級→三倍満級…／§4.5.2・満貫級が下限）。
  評価に応じて **継承（メタ通貨 `wallet.meta`）＋ソウル** を付与し、**`records.tournamentsWon++`**（＝シナリオ `tournament_won` を解放）。
  最終 HP は `avatarHpCurrent` へ持ち越し（`applyTournamentResult`）。敗退/撤退は付与なし。

**未実装（今後）**: 大会の**複数日消費**（§4.5.3）、**決勝戦の本気対局化**（B1 の `launchHonestMatch` を最終戦に差す）、
**周回クリア評価**（§4.5.2 の日数×勝率→引退/見送り→継承）の本実装、複数大会・`mobLvBand` 連動・しきい値チューニング。
