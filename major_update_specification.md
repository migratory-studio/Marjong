...
##### 決定値（チューニング・実装時に確定）
- 持ち点: 単発の本気（4人）＝**固定 25,000**（勝敗・点差を見る）／**師匠タイマン＝弟子の現 HP を賭け、結果を HP に反映**（点棒＝HP）／大会＝節ごと 25,000 リセット（§4.6.10）。
...
- **節＝半荘**（25000 持ち点）。毎節は卓に `unitsAtTable` ユニットが着卓し、**弟子は必ず参加・他ユニットはローテーションで入れ替わる**（`seatUnitsFor`）。
  個人＝`launchHonestMatch`、ペア＝`launchPairTournamentMatch`（2 対 2）、団体＝`launchTeamTournamentMatch`（4 チーム）。結果は `honestCtx.onResult` でユニット順位を返す（pair/team の対局終了に大会分岐を追加）。
- **採点**：各節 **素点((ユニット最終−base)/1000)＋順位点ウマ**（`leaguePoints(standings, uma, base)`）。`base`＝25000×unitSize（ペア50000/団体75000）。
  ウマ＝卓のユニット数で決定（`UMA_BY_UNITS`：4組=[+50/+10/△10/△30]、2組=[+15/△15]）。**卓に居ないユニットは別卓扱いで擬似ポイント加算**（`simAbsentLeaguePt`）＝全員の累積が毎節動く。
...
- **クリア（D）**：全節終了後の**最終累積順位（ユニット）**で評価（`applyLeagueResult`）。順位→**評価ランク**（満貫級が下限・8位も満貫級にクランプ・§4.5.2）＋**継承（`wallet.meta`）＋ソウル**。
  **最終1位＝優勝**で **`records.tournamentsWon++`** ＋ `records.treasures` に宝 id 記録（＝シナリオ `tournament_won` 解放）。**失敗なし**（持ち点は節ごと 25000 でリセット）。
