...
無双国書の会場形式はキャラ別 `finalFormat`（詩玥＝pair／ビビ＝team／ルイナ＝solo4）。CUP ボタンに次の宝名・形式・ティアを表示。
**実装済みラン**: `solo4` / `solo3` / **`pair`** / **`team`**（および無双国書の `finalFormat`＝pair/team/solo4）すべて対応。`runnable` は全形式 true。

**宝の大会＝M リーグ制・ユニット制（全手動・実装版）**（ホーム CUP「宝への道」から）。リーグは常に **8 ユニット** で競う。
- **出場者数を形式で固定**（`ENTRANTS_BY_FORMAT`）：**個人 8 名（1 人×8）/ ペア 16 名（2 人×8）/ 団体 24 名（3 人×8）**。無双国書は会場形式に追従。
...
- **節＝半荘**。毎節は卓に `unitsAtTable` ユニットが着卓し、**弟子は必ず参加・他ユニットはローテーションで入れ替わる**（`seatUnitsFor`）。
  個人＝`launchHonestMatch`、ペア＝`launchPairTournamentMatch`（2 対 2）、団体＝`launchTeamTournamentMatch`（4 チーム）。結果は `honestCtx.onResult` でユニット最終持ち点を返す（pair/team の対局終了に大会分岐を追加）。
- **採点**：各節 **素点＝(ユニット最終−自分の開始HP)/1000**（＝その節の稼ぎ）。**ウマは「増減（稼ぎ）の順位」で配る**（`UMA_BY_UNITS`：4組=[+50/+10/△10/△30]、2組=[+15/△15]）。
