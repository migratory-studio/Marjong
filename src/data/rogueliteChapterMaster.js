...
//
// ⚠️ 章立て（2026-06-24 確定 → 2026-07-10 大2章開通）:
//   大1章＝「師匠をめぐる群像」一本に集約。弟子(詩玥/凌雲/真守)＋御庭番(姚玖/春嬋)＋今は亡き師匠
//          （先代九蓮宝士＝詩玥の恩師）の物語すべてが、この一つの記憶に入る。
//   大2章＝「炎を賭す者たちの記憶」（焔/ルイナ/ドラニエル＝系譜の外の賭け手たち）。
//          プロット正典＝docs/roguelite-ch2-scenario-plot.md。ビートは rogueliteBeatMaster.js。
//
// データ形：
...
export const ROGUELITE_CHAPTER_MASTER = [
  ...
  },
  {
    ...
    title: "炎を賭す者たちの記憶",
    subtitle: "第二の記憶",
    blurb: "賭場の伝説、自称天使、燃えさかる打ち手。師も系譜も持たぬ賭け手たちの記憶が、灰の下で熾きている。——見る者のいなかった炎を、あなたが初めて読む。",
    aim: "灰の層をたどり、賭けても減らない火の在り処へ。",
    ...
    // ボス倍率 1.5→0.7（2026-07-11 必勝制対応）：ボス戦が「勝ち抜くまで終わらない」ルールになり、
    // 4連ボスゲート自体が壁になったため、HPは「上回れる的」へ（グローバル基礎0.8×0.7＝実効0.56）。
    // 実測（N=1200・必勝制）：F40踏破 無宝珠72.8%／若干宝珠78.8%（+6pt）／流派スプレッド1.12。
    bossHpMul: 0.7,
    // tone=ash の意味を再定義（2026-07-10）：「未綴＝沈黙」→「賭けて燃え尽きた者たちの灰。その下の熾火」。
    tone: "ash",
  },
];
